...
// All returns an itr8.Seq that can be used to range over the Set.
func (s *Set[A]) All() itr8.Seq[A] {
...
func FlatMap[A, B comparable](s Set[A], f fn.Function[A, Set[B]]) Set[B] {
	...
	mapper := func(a A) itr8.Seq[B] {
		s := f(a)
		return s.All()
	}

	itr8.FlatMap(s.All(), mapper).DrainTo(&result)
...
func Map[A, B comparable](s Set[A], f fn.Function[A, B]) Set[B] {
	...
	itr8.Map(s.All(), f).DrainTo(&result)
